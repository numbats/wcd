---
title: 'ETC5512: Instructions for Tutorial 9'
author: "Di Cook"
date: "Week 9"
output: 
  html_document:
...
alltime_stats <- html_table(tbl[[5]], fill=TRUE)
```

b. How can we know that the data is organised as a table in the html? Go to the web site in your Google Chrome browser and use the developer tool "Inspect elements" to examine the components of the html.

c. How did we know to extract the data from the 6th table? Try changing the `5` in the code `html_table(tbl[[5]], fill=TRUE)` and compare the result with the tables presented in the web page.
