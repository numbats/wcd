...
**However,** *she must think first before infilling missing data. She should consider the structure of her data, the sample and the population. Knowledge of the sample should inform how you should infill in the missing values.*
...
On the Moodle in Week 10 you will find the author data set `author_df_llm.csv`. This data set contains the guesses from an LLM model about the authors nationalities. Import this data into R and examine the nationality columns.
...
You may like to compare with the web-scraped nationalities.
...
*It seems we can be fairly confident about 282 of the authors, with another 7 having a fuzzy match. There are 70 authors that don't match at all though. Does this mean the LLM got them wrong or the web scraping approach was bad?*

*Looking at mismatches shows there is more work to do. The web-scraped approach hasn't handled the missing values or the nationalities that weren't matched.*
