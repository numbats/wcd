...
* A selective breeding experiment with .monash-blue[107 wheat varieties] (or *genotypes*) were conducted in South Austrlia in a field with plots laid out in a rectangular array with 22 rows and 15 columns.
...
* Although ANOVA is still used today, it is widely recognised as a special case of linear models.
...
* The varieties `VF655`, `TINCURRIN` and `WW1477`  have a .monash-blue[**replication**] of 6, the remaining 104 varieties each have a replicaton of 3. 
...
* Why don't we order the treatments in a **systmatic order** like on the left?
...
ggplot(gilmour.serpentine, aes(col, row)) + 
  ...
  geom_tile(data = subset(gilmour.serpentine, gen=="VG878"), color = "red", size = 1) + 
...
* **Treatments** are the 3 grafiting methods. 
...
* The treatment of repetition as replication in the analysis is referred to as .monash-blue[**pseudoreplication**].
...
6. Australia started to go into lockdown from Sun 22/03/2020. The shutdown measures were effective.
...
* Many underlying health conditions, such as hypertension, is prevelant in elderly.
...
Other than a "small and controlled" cluster of community transmission in Sydney, cases were largely confined to returned travellers.<br><br>
...
<br>
* RCT and NFIP trial sampled from school districts with similiar exposures to the polio virus. 
