...
-   Often we can only collect data about a subset of the population.
...
Get accurate information from the data sample in order to answer questions about the population.
:::
...
-   what **responses** (dependent variables) or **covariates**
    (explanatory or independent or predictor variables) do you need and
    how will you measure those (e.g. Do you collect their age? What age range
...
:::{.callout-warning style="width:100%; background-color:white;"}
## Concept check
What might go wrong with a simple random sampling of 10 creatures from this
population?
:::
...
![](images/lecture-02/art03.png){width="100%" style="float:left;"}
...
::: {.callout-important}
## Designing a data collection is **_hard_**
...
An **observational unit** is not the **observation** (the response)! (e.g. The wheat yield, flu status, and purchase decisions are the observations, not the observational units.)
    
Also note, sometimes the experimental units *are the same as* the observational units.
...
## French fries example (hot chips)
...
Systematic designs are prone to [**bias**]{style="color:#004C9F;"} and [**confounding**]{style="color:#004C9F;"} - so we want to avoid that! 
...
:::incremental 
  ...
  * [**other bias**]{style="color:#004C9F;"} - e.g. the lab technician giving the treatment to the first rat that is taken out of the cage (it's easiest to catch) 
...
Why is the rate of polio cases different?
...
Basically, designing and running experiments is *hard*.
...
We need to knowing how the sample of data relates to the population.

Otherwise it is difficult to make [inferential statements]{style="color:#004C9F;"} and decisions with the data.
